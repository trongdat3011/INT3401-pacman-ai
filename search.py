...
def depthFirstSearch(problem):
    ...
    while (not stack.isEmpty()):
        state, dist, actionList = stack.pop()
        if (problem.isGoalState(state)):
            return actionList
        visitted.add(state)
        successors = problem.getSuccessors(state)
        for nextMove in successors:
            next_state, next_action = nextMove[0], nextMove[1]
            if (next_state not in visitted):
                newActionsList = actionList[:]
                newActionsList.append(next_action)
                stack.push( (next_state, dist+1, newActionsList)  )
...
def breadthFirstSearch(problem):
    ...
    while (not queue.isEmpty()):
        state, dist, actionList = queue.pop()
        if (problem.isGoalState(state)):
            return actionList
        visitted.add(state)
        successors = problem.getSuccessors(state)
    ...
    while (not heap.isEmpty()):
        ...
        for nextMove in successors:
            ...
            if (next_state not in visitted):
                ...
                newActionsList.append(next_action)
                heap.push( (next_state, dist+path_cost, newActionsList), dist+path_cost  )
